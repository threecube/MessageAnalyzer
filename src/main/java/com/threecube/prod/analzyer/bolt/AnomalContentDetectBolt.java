...
/**
 * bolt for detecting messages which contain abnormal key words 
 * 
 ...
 */
@Slf4j
public class AnomalContentDetectBolt extends AbstractIRichBolt{
	 ...
	 */
	public AnomalContentDetectBolt(GwDetectorInterface gwDetectorHandler) {
	...
	public void execute(Tuple input) {
		...
		// generate statModel
		GwStatisticDO gwStatDO = StatDataConvertor.genModelB(sender, content, dimension, count);
				
		//3. detect data
		DetectResultModel detectResult = gwDetectorHandler.detect(gwStatDO);
	
		//4. post process
		if(detectResult != null) {
			
			//emit statistic data to mysql bolt
			this.collector.emit(JDBC_STREAM_ID, new Values(TableMapperEnum.GW_STATTSTIC, gwStatDO));
			...
			if(detectResult.isValid()) {
				...
				this.collector.emit(REDIS_STREAM_ID, new Values(detectResult.getAnormalUserId(), 
						null, detectResult.getOperationCode(),
						detectResult.getRuleType()));
			}
			
			//send filter result
			GwUserFilterDO gwUserFilterDO = GwUserFilterConvetor.Model2DO(detectResult, gwStatDO);
